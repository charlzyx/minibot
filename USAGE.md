...
- **Docker** >= 20.10.0 - 用于 `/code` 命令的容器隔离

如果未安装 Docker，`/code` 命令会返回错误提示。
...
`/code` 命令使用 **Docker 容器隔离** 来执行代码任务：

- 每次执行都创建**全新的独立容器**
- 容器具有**资源限制**（内存 512MB，CPU 50%）
- 容器执行完成后**自动删除**
- **网络隔离**（默认禁用网络，可通过配置开启）
- **只读文件系统**，只能访问指定的输入输出目录

**隔离保证**：
- 容器内无法访问宿主机文件系统（除挂载的卷）
- 容器内进程无法影响宿主机或其他容器
- 容器崩溃不影响主服务
- 执行超时（60秒）自动清理
